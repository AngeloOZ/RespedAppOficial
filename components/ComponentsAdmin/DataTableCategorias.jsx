...
const DataTableCategorias = ({ categories }) => {
    ...
    >
      <div align="center">
        ...
        <TextField
          ...
          onChange={handleChange}
          name="NAME"
          value={categoriaSeleccionade && categoriaSeleccionade.NAME}
        >
          {" "}
        </TextField>
    ...
    >
      <div align="center">
        <p>
          ...
          <b>{categoriaSeleccionade && categoriaSeleccionade.NAME}</b> ?{" "}
        </p>
  ...
  const peticionDelete = async () => {
    await axios
      .delete(url + categoriaSeleccionade.IDCATEGORIA)
      .then((response) => {
        mutate("/categoria");
        SweetAlert.success({
        ...
        setData(
          data.filter(
            (categoria) =>
              categoria.IDCATEGORIA !== categoriaSeleccionade.IDCATEGORIA
          )
        );
  ...
  const peticionPost = async () => {
    await axios.post(url, categoriaSeleccionade).then((response) => {
      mutate("/categoria");
      SweetAlert.success({
  ...
  const peticionPut = async () => {
    await axios.put(url, categoriaSeleccionade).then((response) => {
      mutate("/categoria");
      SweetAlert.success({
            ...
            {Object.entries(categories).map((categoria) => (
              <TableRow key={categoria[1].IDCATEGORIA}>
                ...
                <TableCell width={100} align="center">
                  <EditIcon
                    onClick={() => seleccionarCategoria(categoria[1], "Editar")}
                  />
                  <DeleteIcon
                    color="error"
                    onClick={() =>
                      seleccionarCategoria(categoria[1], "Eliminar")
                    }
                  />
                </TableCell>
              </TableRow>
