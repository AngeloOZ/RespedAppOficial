...
const ListPedidosConfirmado = ({pedidosLocales,pedidosDomicilio,pedidosReserva}) => {
   
   return (
      <div>
            {
               pedidosLocales.map(pedido => (
                  (pedido.IDSTATE == 2) ? <> <Paper sx={{margin:2}}> <ListItem key={pedido.IDPEDIDO}>
                     <LocalDiningIcon/>
                     ...
                     <Button variant="outlined" color="secondary" sx={{margin:1}}>Preparar</Button>
                     </div> </Paper> </>: null
               ))
            }
            {
...
export default ListPedidosConfirmado;